...
async function main() {
  ...
  try {
    const duplicates = await prisma.$queryRaw<Array<{ orangehrm_employee_id: number; count: bigint; application_ids: string[] }>>`
      SELECT
  ...
  try {
    const duplicates = await prisma.$queryRaw<Array<{ orangehrm_employee_id: number; count: bigint; user_ids: string[] }>>`
      SELECT
        orangehrm_employee_id,
        COUNT(*) as count,
