...
export async function syncJobPostingToFrappe(
  ...
  function findBestDesignation(title: string | null | undefined): string {
    ...
    if (lower.includes("designer") || lower.includes("ui") || lower.includes("ux"))
      return "Designer";
    if (lower.includes("hr") || lower.includes("human resource") || lower.includes("recruiter"))
      return "HR Manager";
    if (lower.includes("accountant") || (lower.includes("finance") && !lower.includes("manager")))
      return "Accountant";
    ...
    if (lower.includes("manager") || lower.includes("lead") || lower.includes("head"))
      return "Manager";
    if (lower.includes("executive") || lower.includes("specialist") || lower.includes("officer"))
  ...
  console.log(
    `${logPrefix} Designation: ${posting.designation || posting.title} → ${mappedDesignation}`,
  );
