...
export async function matchDesignation(
  ...
  if (topMatch.rating >= FUZZY_THRESHOLD) {
    console.log(
      `${logPrefix} Designation: "${trimmedInput}" → fuzzy_match (${topMatch.rating.toFixed(2)}) → "${topMatch.target}"`,
    );
    return {
  ...
  if (autoCreate) {
    console.warn(
      `${logPrefix} Designation: "${trimmedInput}" → NO MATCH → auto_created → "${trimmedInput}" (creating new record)`,
    );
    ...
    try {
      ...
      throw new Error(
        `Failed to create new Designation "${trimmedInput}": ${error instanceof Error ? error.message : String(error)}`,
      );
    }
  } else {
    console.warn(
      `${logPrefix} Designation: "${trimmedInput}" → NO MATCH → held_for_review (AUTO_CREATE_RECORDS=false)`,
    );
    return {
...
export async function matchDepartment(
  ...
  if (topMatch.rating >= FUZZY_THRESHOLD) {
    console.log(
      `${logPrefix} Department: "${trimmedInput}" → fuzzy_match (${topMatch.rating.toFixed(2)}) → "${topMatch.target}"`,
    );
    return {
      mappedValue: topMatch.target,
      confidence: "fuzzy_match",
  ...
  if (autoCreate) {
    ...
    console.warn(
      `${logPrefix} Department: "${trimmedInput}" → NO MATCH → auto_created → "${departmentName}" (creating new record)`,
    );
    ...
    try {
      ...
      throw new Error(
        `Failed to create new Department "${trimmedInput}": ${error instanceof Error ? error.message : String(error)}`,
      );
    }
  } else {
    console.warn(
      `${logPrefix} Department: "${trimmedInput}" → NO MATCH → held_for_review (AUTO_CREATE_RECORDS=false)`,
    );
    return {
      mappedValue: "",
      confidence: "held_for_review",
...
export function matchEmploymentType(
  ...
  if (exactMatch) {
    console.log(
      `${logPrefix} Employment Type: "${trimmedInput}" → exact_match (normalized "${normalizedInput}") → "${exactMatch}"`,
    );
    return {
      mappedValue: exactMatch,
      confidence: "exact_match",
