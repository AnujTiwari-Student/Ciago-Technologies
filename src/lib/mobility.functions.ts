...
export const listInternalJobs = createServerFn({ method: "GET" })
  ...
  .handler(async ({ data, context }): Promise<InternalJob[]> => {
    let q = context.supabase.from("job_postings").select("*").in("status", ["published", "internal_only"]);
...
export const listMyReports = createServerFn({ method: "GET" })
  ...
  .handler(async ({ context }) => {
    ...
    const { data: myRoles } = await context.supabase
      .from("user_roles").select("role, department_id").eq("user_id", context.userId);
    const deptIds = Array.from(new Set((myRoles ?? []).map((r: any) => r.department_id).filter(Boolean)));
