...
// Originally auto-generated by Lovable and verified verbatim in the
// `legacySupabaseAuthBranch` function below.
//
...
//   the codepath is the original one supplied by the Lovable toolchain — the
//   Bearer token is a Supabase-issued JWT, validated via supabase.auth.getClaims,
//   and the per-user Supabase client is constructed from that token.
//
...
function requireEnv(name: string): string {
  ...
  if (!value) {
    const message = `Missing environment variable: ${name}. Connect Supabase in Lovable Cloud.`;
