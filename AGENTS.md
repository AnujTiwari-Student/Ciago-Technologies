...
A discipline layer for running Claude Code sessions efficiently when the workflow deliberately splits work between **Opus** (architecture, planning, hard reasoning) and **Sonnet 4.6** (implementation, routine edits). This skill governs *how* Claude Code uses its own built-in tools — it doesn't replace them.
...
- It does not skip real verification steps (e.g. re-reading a file *after* an edit to confirm it landed).
- It does not override an explicit user instruction to switch models or effort levels — it only avoids *unnecessary* switches.
